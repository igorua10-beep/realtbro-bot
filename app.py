...
FastAPI + Aiogram 3.x + Qdrant + OpenAI Embeddings
"""
...
async def search_clients(query: str, top_k: int = TOP_K, status_filter: str = "активна") -> list[dict]:
...
def format_results(results: list[dict], query: str) -> str:
    ...
    lines = [f"🏠 Топ-{len(results)} клієнтів:\n«{query}»\n"]
...
async def _do_search(message: Message, query: str, top_k: int = TOP_K) -> None:
    if any(w in query.lower() for w in ARCHIVE_WORDS):
        ...
        status = "активна"
        await message.answer("🔍 Шукаю серед активних клієнтів…")
    try:
        results = await search_clients(query, top_k=top_k, status_filter=status)
        await message.answer(format_results(results, query))
    except Exception as exc:
